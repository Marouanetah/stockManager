...
if($result->num_rows > 0) {
 ...
 while($row = $result->fetch_array()) {
 	...
 	// active
 	if($row[4] == 1) {
 		$paymentStatus = "<label class='label label-success'>Totalité</label>";
 	} else if($row[4] == 2) {
 		$paymentStatus = "<label class='label label-info'>Acompte</label>";
 	} else {
 		$paymentStatus = "<label class='label label-warning'>Pas de Paiement</label>";
 	} // /else
	...
	<div class="btn-group">
	  ...
	  <ul class="dropdown-menu">
	    ...
	    <li><a type="button" data-toggle="modal" data-target="#removeOrderModal" id="removeOrderModalBtn" onclick="removeOrder('.$orderId.')"> <i class="glyphicon glyphicon-trash"></i> Retirer</a></li>
	  </ul>
	</div>';
