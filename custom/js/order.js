...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			// form validation
			if(orderDate == "") {
				$("#orderDate").after('<p class="text-danger"> Le champ Date de commande est obligatoire </p>');
			...
			if(clientName == "") {
				$("#clientName").after('<p class="text-danger"> Le champ Nom de client de contact est obligatoire </p>');
			...
			if(clientContact == "") {
				$("#clientContact").after('<p class="text-danger"> Le champ Télephone est obligatoire </p>');
			...
			if(paid == "") {
				$("#paid").after('<p class="text-danger"> Le champ Montant Payé est obligatoire</p>');
			...
			if(discount == "") {
				$("#discount").after('<p class="text-danger"> Le champ Remise est obligatoire </p>');
			...
			if(paymentStatus == "") {
				$("#paymentStatus").after('<p class="text-danger"> Le champ Statut de Paiement est obligatoire </p>');
		    ...
		    if(productName[x].value == ''){
		    	$("#"+productNameId+"").after('<p class="text-danger"> Le champ Produit est obligatoire </p>');
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
			...
			if(clientName == "") {
				$("#clientName").after('<p class="text-danger"> Le champ Nom de Client est obligatoire </p>');
				$('#clientName').closest('.form-group').addClass('has-error');
			} else {
				$('#clientName').closest('.form-group').addClass('has-success');
			} // /else
	...
	if(orderId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				var mywindow = window.open('', 'Elkhaloufi Vision', 'height=400,width=600');
...
function getProductData(row = null) {
	if(row) {
		...
		alert('Pas de rangée! veuillez rafraîchir la page');
	}
} // /select on product data
...
function subAmount() {
	...
	// vat
	//modify this to return the TVA value "(Number($("#subTotal").val())/100) * 13;"
	var vat = 0;
	vat = vat.toFixed(2);
	$("#vat").val(vat);
	$("#vatValue").val(vat);
		...
		$.ajax({
			...
			success:function(response) {
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(payAmount == "") {
						$("#payAmount").after('<p class="text-danger">Le champ Montant de la paye est requis!</p>');
					...
					if(paymentType == "") {
						$("#paymentType").after('<p class="text-danger">Le champ Type de Paiement est requis!</p>');
						$("#paymentType").closest('.form-group').addClass('has-error');
					} else {
						$("#paymentType").closest('.form-group').addClass('has-success');
					}
					...
					if(paymentStatus == "") {
						$("#paymentStatus").after('<p class="text-danger">Le champ Statut de Paiement est requis!</p>');
						$("#paymentStatus").closest('.form-group').addClass('has-error');
					} else {
						$("#paymentStatus").closest('.form-group').addClass('has-success');
					}
